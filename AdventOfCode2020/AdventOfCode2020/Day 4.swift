...
struct Day4 {
    ...
    func solve() -> Int {
        ...
        input.forEach { passport in
            let passportData = passport.components(separatedBy: " ")
            
            guard passportData.count == 7 || passportData.count == 8 else { return }
            
            for key in requiredKeys {
                guard !passportData.contains(where: { s -> Bool in
                    s.starts(with: key)
                }) else { return}
            }
    ...
    func solve2() -> Int {
        ...
        input.forEach { passport in
            ...
            for datafield in passportData {
                let parts = datafield.components(separatedBy: ":")
                
                switch parts[0] {
                case "byr":
                    guard parts[1].isValidYear(between: 1920, and: 2002) else { return }
                    break
                case "iyr":
                    guard parts[1].isValidYear(between: 2010, and: 2020) else { return }
                    break
                case "eyr":
                    guard parts[1].isValidYear(between: 2020, and: 2030) else { return }
                    break
                case "hgt":
                    guard parts[1].isValidHeight else { return }
                    break
                case "hcl":
...
fileprivate extension String {
    func isValidYear(between start: Int, and end: Int) -> Bool {
        guard self.count == 4, let year = Int(self) else { return false }
        return year >= start && year <= end
    }
    
    var isValidHeight: Bool {
        let containsCMMark = self.contains("cm")
        guard containsCMMark || self.contains("in") else { return false }
        
        let remainder = Int(self.replacingOccurrences(of: containsCMMark ? "cm" : "in", with: ""))!
        
        if containsCMMark {
            return remainder >= 150 && remainder <= 193
        } else {
            return remainder >= 59 && remainder <= 76
        }
    }
