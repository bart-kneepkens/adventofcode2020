...
extension String {
    func occurenceCount(_ character: Character) -> Int {
        self.filter({ $0 == character }).count
    }
}
...
struct Input {
    ...
    init(_ string: String) {
        ...
        self.range = Int(rangeComponents[0])!...Int(rangeComponents[1])!
        self.specifier = components[1].trimmingCharacters(in: .punctuationCharacters)
        self.password = components[2]
    }
    
    var isValid: Bool {
        let count = self.password.occurenceCount(self.specifier.first!)
        return self.range.contains(count)
    }
    
    var isValidPart2: Bool {
        let arr = Array(self.password)
        
        let lower = self.range.lowerBound - 1
        let upper = self.range.upperBound - 1
        
        let worksForLower = arr[lower] == specifier.first!
        let worksForUpper = arr[upper] == specifier.first!
        
        return (worksForLower && !worksForUpper) || (!worksForLower && worksForUpper)
    }
}

struct Day2 {
    func solve() {
        print(input.filter({ $0.isValid }).count)
    }
    
    func solve2() {
        print(input.filter({ $0.isValidPart2 }).count)
    }
}
