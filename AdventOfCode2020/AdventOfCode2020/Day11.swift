...
struct Day11 {
    ...
    typealias RulesClosure = (([[String]]) -> [[String]])
    
    static func findOccupiedSeatsAfterChangesHaveStopped(_ applyRules: RulesClosure) -> Int {
        let inputFerry = input.components(separatedBy: .newlines).map({ Array($0.map({ String($0) })) })
        
        var hasChanges = true
        
        var ferry = inputFerry
        
        while hasChanges {
            let next = applyRules(ferry)
            hasChanges = ferry != next
            ferry = next
        }
        
        return ferry.map { arr -> Int in
            return arr.filter({ $0 == OCCUPIED}).count
        }.reduce(0, +)
    }
    
    static func solve() -> Int {
        func applyRules(_ ferry: [[String]]) -> [[String]] {
            var copy = ferry
            
            for (y, row) in ferry.enumerated() {
                for (x, seat) in row.enumerated() {
                    ...
                    let adjacentOccupiedCount = DIRECTIONS.map { direction -> Bool in
                        let xx = x + direction.1
                        let yy = y + direction.0
                        
                        guard xx > -1, yy > -1,
                              yy < ferry.count,
                              xx < ferry[yy].count else {
                            return false
                        }
                        
                        return ferry[yy][xx] == OCCUPIED
                    }
                    .filter({ $0 })
                    .count
                    ...
                    if seat == OCCUPIED && adjacentOccupiedCount >= 4 {
                        copy[y][x] = EMPTY
                    }
                }
            }
        ...
        return findOccupiedSeatsAfterChangesHaveStopped(applyRules)
    }
        ...
        func applyRules(_ ferry: [[String]]) -> [[String]] {
            ...
            for (y, row) in ferry.enumerated() {
                for (x, seat) in row.enumerated() {
                    ...
                    let adjacentOccupiedCount = DIRECTIONS.map { direction -> Bool in
                        var xx = x + direction.1
                        var yy = y + direction.0
                        
                        while yy > -1 && xx > -1 && yy < ferry.count && xx < ferry[yy].count {
                            
                            if ferry[yy][xx] == OCCUPIED {
                                return true
                            }
                            
                            if ferry[yy][xx] == EMPTY {
                                return false
                            }
                    ...
                    if seat == OCCUPIED && adjacentOccupiedCount >= 5 {
                        copy[y][x] = EMPTY
                    }
                }
            }
