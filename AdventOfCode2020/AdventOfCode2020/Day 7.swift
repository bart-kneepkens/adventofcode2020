...
class Bag {
    ...
    var count: Int {
        return 1 + adjacent.reduce(0) { $0 + $1.1 * $1.0.count }
    }
    
    func depthFirstSearch(to destination: Bag) -> Bool {
        guard self.color != destination.color else { return true }
        return !self.adjacent.map({ $0.0.depthFirstSearch(to: destination) }).filter({ $0 == true }).isEmpty
    }
}

var inputDictionary: [String: Bag] {
    var dictionary = [String: Bag]()
    for inputLine in input {
        let components = inputLine .trimmingCharacters(in: .punctuationCharacters).components(separatedBy: "bags contain")
        let color = components[0].trimmingCharacters(in: .whitespaces)
        
        let bag = dictionary[color, default: Bag(color: color)]
        dictionary[color] = bag
        
        guard !components[1].contains("no other") else { continue }
        
        let containedStrings = components[1].components(separatedBy: ", ")
        
        for content in containedStrings {
            let scanner = Scanner(string: content)
            let num = scanner.scanInt()!
            let color = scanner.scanUpToString(" bag")!
            let sub = dictionary[color, default: Bag(color: color)]
            dictionary[color] = sub
            
            bag.adjacent.append((sub, num))
        }
    }
    return dictionary
}
...
struct Day7 {
    static func solve() -> Int {
        let goldenBag = inputDictionary["shiny gold"]!
        
        return inputDictionary.values.map { bag -> Int in
            guard bag.color != goldenBag.color else { return  0 }
            return bag.depthFirstSearch(to: goldenBag) ? 1 : 0
        }.reduce(0, +)
    }
    
    static func solve2() -> Int {
        return inputDictionary["shiny gold"]!.count
    }
}
