...
struct Day14 {
    ...
    static func solve() -> Int {
        ...
        for instruction in instructions {
            if instruction.isMaskInstruction {
                currentMask = instruction.mask
            } else {
                let memAssignmentString = String(instruction.memoryAssignment, radix: 2)
                let frontPadding = String(repeating: "0", count: 36 - memAssignmentString.count)
                let padded = "\(frontPadding)\(memAssignmentString)"
                let result = padded.applyMask(currentMask)
                
                mem[instruction.memoryAddress] = Int(result, radix: 2)
            }
        }
...
fileprivate extension String {
    var isMaskInstruction: Bool {
        return self.starts(with: "mask")
    }
    
    var mask: String {
        guard isMaskInstruction else { fatalError() }
        return self.replacingOccurrences(of: "mask = ", with: "")
    }
    
    var memoryAddress: String {
        guard !isMaskInstruction else { fatalError() }
        let arr = Array(self)
        return String(arr[(arr.firstIndex(of: "[")! + 1)..<(arr.firstIndex(of: "]")!)])
    }
    
    var memoryAssignment: Int {
        guard !isMaskInstruction else { fatalError() }
        let arr = Array(self.components(separatedBy: "= ")[1].trimmingCharacters(in: .whitespaces))
        return Int(String(arr))!
    }
    
    func applyMask(_ mask: String) -> String {
        guard mask.count == self.count else { fatalError() }
        
        var arr = Array(self)
        mask.enumerated().forEach { index, char in
            guard char != "X" else { return }
            arr[index] = char
        }
